...
## Project Layout
- `pythonProject/mouse_center_lock_gui.py` – GUI app (PySide6)
- `pythonProject/mouse_center_lock.py` – CLI/basic version (optional)
- `pythonProject/i18n/` – language files
- `pythonProject/assets/` – icons and assets (put `app.ico` here)
- `pythonProject/config.json` – default config (user override is written next to the exe)
...
Run:
```bash
python pythonProject/mouse_center_lock_gui.py
```
...
pyinstaller --noconfirm --clean --windowed \
  ...
  --add-data "pythonProject/config.json;." \
  --add-data "pythonProject/assets;assets" \
  pythonProject/mouse_center_lock_gui.py
```
The exe will be in `dist/MouseCenterLock/MouseCenterLock.exe`.
