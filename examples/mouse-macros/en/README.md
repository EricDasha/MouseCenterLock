...
- `far-far-west-back-left-combo.json`: `x1` toggles armed mode, left press fires → left `mouseDown`, 60ms, `mouseUp`, 120ms, KeyDown `2`, 80ms, KeyDown `1`, 27ms, KeyUp `2`, 66ms, KeyUp `1`; cancel cleanup sends `KeyUp 2/1`. Recommended `cooldownMs=450` to avoid weapon-swap overlap.
