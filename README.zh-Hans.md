...
如果目标窗口不接收“瞬时点击”，可以在高级设置里把 **Mouse down hold (ms)** 调到 `8` ~ `20`，让每次点击变成“按下 → 保持数毫秒 → 松开”。
...
高级设置里的「输入后端」由连点器和宏输出共用；如果目标程序无视瞬时点击，可先尝试 `native-sendinput` 并给连点器设置 8-20ms 的「鼠标按住时长」，再尝试 `window-message`。
