...
### v1.1.0
- 新增连点器方案管理，支持创建、切换、保存与删除多组连点设置。
- 新增 Windows 通知播报，优先使用原生 toast，失败时回退到托盘提示。
- 新增连点器启动音效，支持内置预设、自定义音频文件与音效试听。
- 新增多种连点触发方式，支持切换启动、按住键盘按键启动、按住鼠标按键启动。
- 连点按键新增支持鼠标中键。
- 默认配置文件改为 `Mconfig.json`，并兼容读取旧版 `config.json`。
