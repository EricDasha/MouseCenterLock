...
- `far-far-west-back-left-combo.json`: hold mouse back side button `x1`, press left → left click, 54ms, KeyDown `2`, 80ms, KeyDown `1`, 27ms, KeyUp `2`, 66ms, KeyUp `1`; cancel cleanup sends `KeyUp 2/1`. Recommended `cooldownMs=450` to avoid weapon-swap overlap.
