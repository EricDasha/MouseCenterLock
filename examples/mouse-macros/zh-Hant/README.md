...
- `repeat-r-on-1-off-2.json`：按 `1` 開始每 100ms 按一次 `R`；按 `2` 停止。
- `left-hold-repeat-r-on-1-off-2.json`：按 `1` 開啟偵測；開啟後按住左鍵 → 每 100ms 按一次 `R`；鬆開左鍵暫停輸出；按 `2` 停止偵測。
...
`panicHotkey` 是應用級設定，不屬於單條規則。預設是 `F12`；按下後會強制停止正在執行/切換中的巨集，並釋放已按下的輸出。
